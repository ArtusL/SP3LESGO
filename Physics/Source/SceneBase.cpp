...
void SceneBase::Init()
{
	...
	meshList[GEO_PLAYER]->textureID = LoadTGA("Image//playerpicture.tga");
	...
	meshList[GEO_SHREK_POINTER] = MeshBuilder::GenerateQuad("Shrek Left", Color(1, 1, 1), 1.f);
	meshList[GEO_SHREK_POINTER]->textureID = LoadTGA("Image//shrekpointer.tga");
	...
	meshList[GEO_UIBG]->textureID = LoadTGA("Image//UI//MainBackground.tga");

	meshList[GEO_DEADBG] = MeshBuilder::GenerateQuad("main menu", Color(1, 1, 1), 1.f);
	meshList[GEO_DEADBG]->textureID = LoadTGA("Image//UI//DEAD.tga");
